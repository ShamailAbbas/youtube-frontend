...
function LikeDislikes(props) {
  ...
  const onLike = () => {
    if (props.userId) {
      if (LikeAction === null) {
        Axios.post('http://localhost:5000/like/upLike', variable).then(
          (response) => {
            if (response.data.success) {
              setLikes(Likes + 1)
              setLikeAction('liked')
        ...
        Axios.post('http://localhost:5000/like/unLike', variable).then(
          (response) => {
            if (response.data.success) {
              setLikes(Likes - 1)
              setLikeAction(null)
  ...
  const onDisLike = () => {
    if (props.userId) {
      if (DislikeAction !== null) {
        Axios.post('http://localhost:5000/like/unDisLike', variable).then(
          (response) => {
            if (response.data.success) {
              setDislikes(Dislikes - 1)
              setDislikeAction(null)
        ...
        Axios.post('http://localhost:5000/like/upDisLike', variable).then(
          (response) => {
            if (response.data.success) {
              setDislikes(Dislikes + 1)
              setDislikeAction('disliked')
